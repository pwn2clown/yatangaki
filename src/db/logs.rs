...
pub fn create_project_db(name: &str) -> Result<(), DbError> {
    ...
    let packet_index = {
        let mut stmt = conn
            .prepare("SELECT MAX(packet_id) FROM requests")
            .map_err(DbError::SqliteError)?;

        let rows = stmt.query([]).map_err(DbError::SqliteError);
        let p = match rows.unwrap().next().unwrap() {
            Some(row) => {
                let max_packet_id: usize = row.get::<usize, usize>(0).unwrap() + 1;
                max_packet_id
            }
            None => 0,
        };
